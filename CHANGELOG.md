...
- Switch template downloads to the zh-local fork by default (owner `iails01/spec-kit`), with environment variable overrides (`SPECIFY_TEMPLATE_REPO` or owner/name vars) to control the source repository.

## [0.0.23] - 2025-11-08

- When initializing with Codex (`--ai codex`), automatically ensure `AGENTS.md` exists (without touching existing files) and starts with a read-only reminder about `.specify/templates` and `.specify/scripts`.
