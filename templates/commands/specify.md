...
用户在触发消息中 `/speckit.specify` 后输入的文本**就是**功能描述。假设你在此对话中始终可以获取它，即使下面字面显示 `{ARGS}`。除非用户提供了空命令，否则不要要求用户重复输入。
...
4. 遵循以下执行流程：
    ...
    6. 定义成功标准
       ...
       每个标准必须能够在不知道实施细节的情况下验证
    7. 识别关键实体（如果涉及数据）
    8. 返回：SUCCESS（规范准备好进行规划）

5. 使用模板结构将规范写入 SPEC_FILE，用功能描述（参数）派生的具体细节替换占位符，同时保留部分顺序和标题。

6. **规范质量验证**：写完初始规范后，根据质量标准进行验证：

   a. **创建规范质量检查表**：在 `FEATURE_DIR/checklists/requirements.md` 生成检查表文件，使用检查表模板结构包含以下验证项：
   ...
   b. **运行验证检查**：根据每个检查表项审查规范：
      - 对于每个项目，确定是否通过或失败
      - 记录发现的具体问题（引用相关规范部分）

   c. **处理验证结果**：

      - **如果所有项目通过**：标记检查表完成并继续步骤 6
      ...
      - **如果 [NEEDS CLARIFICATION] 标记仍然存在**：
        ...
        2. **限制检查**：如果存在超过 3 个标记，仅保留 3 个最关键的（按范围/安全/用户体验影响排序），对其余做出明智猜测
        3. 对于每个需要澄清的问题（最多 3 个），以此格式向用户展示选项：
           ...
           **我们需要知道什么**：[NEEDS CLARIFICATION 标记中的具体问题]
        ...
        6. 在等待响应之前一起展示所有问题
        7. 等待用户对所有问题的选择作出响应（例如，"Q1: A, Q2: 自定义 - [详情], Q3: B"）
        8. 通过用用户选择或提供的答案替换每个 [NEEDS CLARIFICATION] 标记来更新规范
        9. 解决所有澄清后重新运行验证

   d. **更新检查表**：每次验证迭代后，用当前通过/失败状态更新检查表文件

7. 报告完成情况，包括分支名称、规范文件路径、检查表结果以及下一阶段的准备情况（`/speckit.clarify` 或 `/speckit.plan`）。
...
5. **像测试人员一样思考**：每个模糊的需求都应该无法通过"可测试且无歧义"检查表项
6. **通常需要澄清的区域**（仅当不存在合理默认值时）：
   - 功能范围和边界（包含/排除特定用例）
   - 用户类型和权限（如果存在多种冲突解释）
   - 安全/合规要求（当法律/财务上重要时）
