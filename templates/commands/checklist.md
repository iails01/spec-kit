...
**隐喻**：如果你的规范是用英语编写的代码，那么检查清单就是它的单元测试套件。你正在测试需求是否编写良好、完整、明确并准备好实施 - 而不是实现是否工作。
...
2. **澄清意图（动态）**：得出最多三个初始上下文澄清问题（没有预先准备好的目录）。它们必须：
   ...
   3. 如果不明确，确定可能的受众和时间（作者，审阅者，QA，发布）。
   4. 检测缺失的维度：范围广度，深度/严谨性，风险重点，排除边界，可衡量的验收标准。
   5. 制定从这些原型中选择的问题：
      - 范围细化（例如，“这应该包括与 X 和 Y 的集成接触点，还是仅限于本地模块的正确性？”）
      - 风险优先级（例如，“这些潜在风险领域中的哪一个应接受强制性关卡检查？”）
      - 深度校准（例如，“这是一个轻量级的预提交健全性列表还是正式的发布关卡？”）
      - 受众框架（例如，“这将仅由作者使用，还是在 PR 审查期间由同行使用？”）
      - 边界排除（例如，“我们应该在本轮中明确排除性能调整项目吗？”）
      - 场景类缺口（例如，“未检测到恢复流程——回滚/部分失败路径是否在范围内？”）

   问题格式规则：
   - 如果提供选项，生成一个包含列的紧凑表格：选项 | 候选 | 为何重要
   - 最多限制为 A–E 选项；如果自由形式的答案更清晰，则省略表格
   - 永远不要要求用户重述他们已经说过的话
   ...
   输出问题（标签 Q1/Q2/Q3）。回答后：如果 ≥2 个场景类（备用/异常/恢复/非功能域）仍不清楚，您可以再问最多两个有针对性的后续问题（Q4/Q5），每个问题附带一行理由（例如，“未解决的恢复路径风险”）。总问题数不要超过五个。如果用户明确拒绝更多，则跳过升级。
...
6. **结构参考**：按照 `templates/checklist-template.md` 中的规范模板生成检查清单，包括标题、元部分、类别标题和 ID 格式。如果模板不可用，请使用：H1 标题，目的/创建元行，包含 `- [ ] CHK### <需求项>` 行的 `##` 类别部分，ID 从 CHK001 开始全局递增。
...
**重要**：每个 `/speckit.checklist` 命令调用都会使用简短、描述性的名称创建一个检查清单文件，除非文件已存在。这允许：
...
- [ ] CHK002 - 测试桌面上的悬停状态工作正常 [Spec §FR-003]
