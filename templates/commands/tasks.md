---
description: 根据可用的设计工件，为功能生成可操作的、按依赖顺序排列的 tasks.md。
handoffs: 
  - label: 分析一致性
...
1. **设置**：从仓库根目录运行 `{SCRIPT}` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于像 "I'm Groot" 这样的参数中的单引号，使用转义语法：例如 'I'\''m Groot'（或者如果可能，使用双引号："I'm Groot"）。
...
4. **生成 tasks.md**：使用 `templates/tasks-template.md` 作为结构，填充：
   ...
   - 第三阶段+：每个用户故事一个阶段（按 spec.md 中的优先级顺序）
...
**测试是可选的**：仅当功能规范中明确请求或用户请求 TDD 方法时才生成测试任务。
...
3. **[P] 标记**：仅当任务可并行化（不同文件，不依赖于未完成的任务）时包含
4. **[Story] 标签**：仅对于用户故事阶段任务是必需的
...
2. **来自合约**：
   ...
   - 如果请求测试：每个合约 → 在该故事阶段实施之前的合约测试任务 [P]
